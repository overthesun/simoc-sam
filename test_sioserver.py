...
async def test_register_sensor_no_subs(sio, sensor_id, sensor_info):
    ...
    sio.emit.assert_awaited_once_with('send-data', to=sensor_id, namespace='/sensor')
...
async def test_register_sensor_2_subs(sio, sensor_id, sensor_info, two_subs):
    ...
    for sub in two_subs:
        sio.emit.assert_any_await('sensor-info', info, to=sub, namespace='/client')
    # check that the server asks the sensor to send data
    sio.emit.assert_awaited_with('send-data', to=sensor_id, namespace='/sensor')
...
async def test_register_client(sio, client_id):
    ...
    sio.emit.assert_any_await('hab-info', sioserver.HAB_INFO, to=client_id, namespace='/client')
    sio.emit.assert_awaited_with('sensor-info', sioserver.SENSOR_INFO,
                                 to=client_id, namespace='/client')
...
async def test_emit_to_subscribers_2_subs(sio, two_subs):
    ...
    await sioserver.emit_to_subscribers('test-event')
    for sub in two_subs:
        sio.emit.assert_any_await('test-event', to=sub, namespace='/client')
